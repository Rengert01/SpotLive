...
import { musics, sessions, albums, musicToAlbums } from '@/db/schema';
import { db } from '@/db';
...
const uploadAlbumBodySchema = z.object({
    ...
    musicIds: z.array(z.object({id: z.string()})),
});
...
const uploadAlbum = async (req: Request, res: Response): Promise<void> => {
    ...
    const { name, public: isPublic, musicIds } = params.data;
